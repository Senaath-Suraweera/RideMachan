...
function updateLoadMoreButton() {
  ...
  if (!section) return;
  const loaded = ALL_BOOKINGS.length;
  if (loaded < totalBookings) {
    section.style.display = "block";
    const btn = document.getElementById("loadMoreBtn");
    if (btn) btn.textContent = `Load More (${loaded} / ${totalBookings})`;
  } else {
...
function isPickedUp(tripStartDate) {
  ...
  try {
    return new Date(tripStartDate) <= new Date();
  } catch {
    return false;
  }
}
...
function applyFilters() {
  ...
  document.querySelectorAll(".booking-card").forEach((card) => {
    ...
    if (accepted && !dots.some((t) => t.includes("Accepted"))) ok = false;
    if (paid && !dots.some((t) => t.includes("Paid"))) ok = false;
    if (pickedUp && !dots.some((t) => t.includes("Pick-up"))) ok = false;
...
function applySorting() {
  ...
  cards.sort((a, b) => {
    if (sortValue === "Date (Newest)") {
      const ad = a.dataset.tripStart || "";
      const bd = b.dataset.tripStart || "";
    ...
    if (sortValue === "Price (High to Low)") {
      const ap = parseFloat(a.dataset.totalAmount) || 0;
      const bp = parseFloat(b.dataset.totalAmount) || 0;
      return bp - ap;
    }
