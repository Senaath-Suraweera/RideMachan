...
function renderVehicle(vehicle) {
  ...
  // Auto-fill the Vehicle Registration input and lock it so the user
  // cannot edit it manually (it now follows the dropdown selection).
  const regInput = document.getElementById("vehiclenumber");

  if (regInput) {
    regInput.value = vehicle.numberplate || "";
    regInput.readOnly = true;
    regInput.setAttribute("readonly", "readonly");
    regInput.style.backgroundColor = "#f1f3f5";
    regInput.style.cursor = "not-allowed";
  }
}
...
function handleSubmitButton() {
  ...
  submitButton.addEventListener("click", async () => {
    ...
    let allChecked = Array.from(checklistItems).every((item) =>
    ...
    if (!issuesFound || !issuesFound.value.trim()) {
      showNotification(
        "Please document any issues or repairs needed!",
        "error",
      );
      return;
    }
    ...
    let inspectionData = {
      ...
      checklist: Array.from(document.querySelectorAll(".checklist-item")).map(
        (item) => ({
    ...
    // Also clear the inner checkbox tick — without this the ✓ stays
    // visible because .checkbox.checked::after renders the mark.
    document
      .querySelectorAll(".checklist-item .checkbox")
      .forEach((cb) => cb.classList.remove("checked"));

    // Clear the selected status button highlight
    document
      .querySelectorAll(".status-btn")
      .forEach((b) => b.classList.remove("selected"));

    // Clear the other form inputs so the next inspection starts clean
    if (issuesFound) issuesFound.value = "";
