...
async function LoadAllTripAvailableDrivers(bookingId) {
  try {
    let response = await fetch(
      `/display/trip/available/drivers?bookingId=${bookingId}`,
    );
...
function filterBookingsByText(searchText) {
  ...
  for (let i = 0; i < AllBookings.length; i++) {
    ...
    if (
      customerLower.includes(inputLower) ||
      vehicleLower.includes(inputLower)
    ) {
      filteredBookings.push(AllBookings[i]);
    }
  }
...
function shouldShowAssignDriver(booking) {
  ...
  return (
    ...
    !instruction.includes("self drive") &&
    !booking.driverName
  );
}
...
function renderActiveDrivers(Alldrivers) {
  ...
  Alldrivers.forEach((driver) => {
    ...
    card.className = "driver-card"; // NEW CLASS
    ...
    card.innerHTML = `
            ...
            <div class="driver-actions">
                <button class="btn btn-outline assign-driver-btn">
                    Assign Driver
                </button>
            </div>
        `;
...
async function AssignBooking(driverId) {
  try {
    ...
    showNotification(
      "Failed to assign booking. See console for details!",
      "error",
    );
  }
}
