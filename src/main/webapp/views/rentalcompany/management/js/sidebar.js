...
class Sidebar {
  ...
  render() {
    ...
    if (!container) {
      console.warn("Sidebar container not found — skipping render.");
      return;
    }
  ...
  logout() {
    if (confirm("Are you sure you want to logout?")) {
      fetch("http://localhost:8080/rentalcompanies/logout", {
        method: "GET",
      })
        .then((response) => {
          // backend now redirects, so handle both cases
          if (response.redirected) {
            window.location.href = response.url;
          } else {
            // if backend returns JSON instead of redirect
            return response.json();
          }
        })
        .then((data) => {
          if (data && data.status === "success") {
            window.location.href = "http://localhost:8080/companylogin.html";
          }
        })
        .catch((err) => {
