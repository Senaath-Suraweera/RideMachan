...
function updateDashboard(data) {
    ...
    try {
        ...
        if (data.stats) {
            ...
            // Weekly Bookings
            const weeklyBookingsElement = document.querySelector('.stat-card:nth-child(2) .stat-value');
            ...
            // Daily Hours
            const dailyHoursElement = document.querySelector('.stat-card:nth-child(3) .stat-value');
        ...
        if (data.monthlyIncomeChart) {
            ...
            updateBarChart('.stat-card:nth-child(1) .bar-chart', data.monthlyIncomeChart);
        }

        if (data.weeklyBookingsChart) {
            console.log('Updating weekly bookings chart:', data.weeklyBookingsChart);
            updateBarChart('.stat-card:nth-child(2) .bar-chart', data.weeklyBookingsChart);
        }

        if (data.dailyHoursChart) {
            console.log('Updating daily hours chart:', data.dailyHoursChart);
            updateBarChart('.stat-card:nth-child(3) .bar-chart', data.dailyHoursChart);
        }
        ...
        if (data.notificationCount !== undefined) {
            ...
            if (bellBadge) {
                ...
                bellBadge.style.display = data.notificationCount === 0 ? 'none' : 'flex';
...
function updateBookingSummary(summary) {
    try {
        ...
        if (statItems.length >= 4) {
            statItems[0].querySelector('.stat-number').textContent = summary.completed || 0;
            statItems[1].querySelector('.stat-number').textContent = summary.pending || 0;
            statItems[2].querySelector('.stat-number').textContent = summary.cancelled || 0;
            statItems[3].querySelector('.stat-number').textContent = summary.total || 0;
        }
