...
function calculateExperience(joinedDate) {
    ...
    const adjustedYears = months < 0 || (months === 0 && now.getDate() < joined.getDate())
        ? years - 1
        : years;
...
function handleProfilePictureChange(event) {
    ...
    reader.onload = function(e) {
        ...
        showSuccess('Profile picture updated. Click "Save Profile" to apply changes.');
    };
...
function populateProfile(driver) {
    ...
    setTextContent('profileFullName', fullName.trim() || 'Driver Name', 'Driver Name');
...
function toggleEditMode(event) {
    ...
    const editableFields = ['firstname', 'lastname', 'email', 'mobilenumber', 'homeaddress', 'licensenumber'];
...
async function saveProfile(editableFields, editButton, editIcon, editAvatarBtn) {
    ...
    if (profilePictureBase64 && profilePictureBase64 !== originalData.profilePicture) {
    ...
    try {
        ...
        if (result.status === 'success') {
            ...
            setTextContent('profileFullName', updatedData.firstname + ' ' + updatedData.lastname);
        ...
        showError('Failed to save profile. Please check your connection and try again.');
        editButton.textContent = originalButtonText;
        editButton.style.cursor = 'pointer';
    }
}
